...
2026-09-14 代码提交时的本地回归：Webhook、入库流水线、历史库、Vault 和分集相关测试共 **153 passed**（2 条依赖弃用警告）；当时尚未切换生产配置。后续部署验收见下节。

## 2026-09-14 Webhook 部署验收摘要

经用户确认后，已启用签名 Webhook 通知；部署前保留回退备份，未升级消息代理或重建应用镜像。详细运维记录和私有配置仅留在服务器与本地，不随本摘要上传。

已完成的线上检查：

- 健康检查正常；无凭据访问返回 HTTP 401，有效凭据返回 HTTP 200。
- 错误 HMAC 与过期 V2 签名均被 HTTP 401 拒绝。
- 一条明确标注“部署验收测试”的通知获得 HTTP 200；严格匹配送达回执后记录为 `SENT`。
- 相同通知重复调用及新进程读取持久化状态均不会再次发送。
- 原有任务与 Markdown 文件内容校验一致；其他消息配置保持一致。
- 应用连接正常，容器健康；通知服务运行稳定，未出现自动重启。

验收使用已部署桥接模块和真实通知接口，测试状态与生产任务隔离，不写入业务数据库。没有重新整理笔记、重发历史通知、调用模型或生成视频。此次未重新进行公网浏览器页面验收。

`SENT` 仅表示接口投递成功，不代表接收者已读；实际收信仍需用户确认。

## 前端静态资源部署检查

部署时不能仅凭首页 HTTP 200 判断成功。目录权限继承为 `700` 时，Nginx 工作进程即使能读到首页，也可能无法进入 `assets` 目录；若静态资源走 SPA 回退，就会收到 HTML 并触发 JS/CSS MIME 报错。

- 两种前端镜像构建均将公开静态目录规范为 `755`、文件规范为 `644`；仅限前端构建产物，不对配置、数据库或笔记目录扩大权限。
- `/assets/` 使用独立规则，读取失败返回 404，不回退到首页；`index.html` 使用 `Cache-Control: no-cache`，其他 SPA 路由仍可正常回退。
- 手动复制或解包静态资源后，也必须核对目标目录权限。容器内临时修复不能代替更新持久化构建或挂载源，重建前应确认修复不会丢失。
- 修改配置前保留备份，先运行 `nginx -t`，通过后热加载；无需为静态文件修复重启后端或消息服务。
- 执行 `python deploy/check_frontend_assets.py http://127.0.0.1:3015/`，逐项验证首页脚本、模块预加载和样式的状态、MIME 类型与正文，并验证不存在的资源返回 404。此脚本只读访问，不调用生成、入库或通知接口。
- 以上脚本不替代浏览器验收，不覆盖全部懒加载资源或业务交互；还应通过实际入口检查页面和开发者工具网络错误。

### 2026-09-14 静态资源权限修复验收

已基于 `e2ca1ac` 完成线上热修复：备份原配置与权限信息，规范前端静态目录和文件权限，校验 Nginx 配置后热加载。静态文件内容哈希保持一致，当前应用容器未重启。

同时基于现有应用镜像构建了仅含静态资源权限和 Nginx 配置变更的补丁镜像，核对其静态文件与线上内容一致，并将部署配置指向补丁镜像；未立即重建当前容器，后续重建将使用持久化修复。

- 首页引用的 5 个 JS/CSS 检查通过。
- 对静态目录全部 60 个 JS/CSS 检查 HTTP 200、正确 MIME 类型及响应正文 SHA256，全部通过。
- 不存在的 JS/CSS 返回 404；首页与 `index.html` 返回 `Cache-Control: no-cache`。
- 应用容器健康、通知桥接服务运行正常；此次未调用生成、入库或通知接口。
- 公网匿名访问返回 401，内置浏览器打开网站被客户端拦截；未绕过访问限制，登录后的浏览器页面验收仍需用户确认。不能把上述服务器端验证表述为公网浏览器验收通过。

具体服务器标识、部署路径、镜像配置和备份仅在私有运维记录保留。


## 入库失败通知与历史筛选

- 生成历史新增「入库中」（排队、整理、发布）和「入库失败」筛选。以每篇笔记最新入库任务为准，不受最近 100 条任务展示上限影响。
- 已入库/未入库仍表示文件是否曾成功入库；已有旧版本的笔记可能同时命中「已入库」与「入库中/入库失败」。文件已发布、仅完成通知失败时，不算入库失败。
- 上传、Hermes 整理、发布失败均提交 `bilinote.archive.failed`，消息包含任务 ID、笔记摘要、失败阶段和原因。不把失败通知的发送成功当作入库成功。
- 部署时先将 Hermes 的 `bilinote-archive` 路由 `events` 更新为同时允许 `bilinote.archive.completed`、`bilinote.archive.failed`（参照示例 YAML），保留原密钥、接收人及其他配置，然后更新桥接服务与应用。只更新应用而未放行新事件，会导致失败通知被忽略。
- 完成通知继续使用旧投递 ID 和 `notification.json`，不重发历史完成消息。失败通知按任务 ID + 尝试次数独立持久化；同一次失败不双发，人工重试再次失败会产生新的失败通知。
- 失败通知送达状态为 UNKNOWN/SENDING 时禁止自动重发，请先核对收信。断电前已持久化为 PENDING 的失败事件会在工作进程恢复后继续提交。
- 新逻辑不批量补发升级前的历史失败任务；这些任务在人工重试后再次失败才会提交新失败事件。若桥接服务本身不可达，界面会保留入库错误并显示失败通知未确认，不承诺必达。
